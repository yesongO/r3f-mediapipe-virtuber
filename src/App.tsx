...
export default function App() {
  ...
  const [statusText, setStatusText] = useState("카메라를 보고 움직여보세요!");
  ...
  const predictWebcam = () => {
      ...
      if (video.readyState >= 2 && video.currentTime !== lastVideoTime) {
          ...
          if (faceLandmarkerRef.current) {
              ...
              setStatusText(`감지 중... (얼굴: ${faceDetected ? '✅' : '❌'},)`);
  ...
  return (
      <div className="app-container">
          <div className="ui-container">
              <button onClick={startWebcam} disabled={loading || webcamStarted}>
                  {loading ? "모델 로딩 중..." : webcamStarted ? "웹캠 실행 중" : "웹캠 시작"}
              </button>
              {webcamStarted && <p className="loading-text">{statusText}</p>}
              {/* 입모양 표정 선택 버튼 */}
              {webcamStarted && (
                <div className="expression-buttons">
                  {expressions.map((exp) => (
                    <button
                      ...
                      className={expression === exp ? "active" : ""}
                    >
                      {exp}
                    </button>
                  ))}
          ...
          <Canvas shadows camera={{ position: [0, 0.6, 0.65], fov: 30, rotation: [0, 0, 0] }}>
              <Suspense fallback={null}>
                  <Physics gravity={[0, -9.81, 0]}>
                      <VroidAvatar rig={rig} blendshapes={blendshapes} expression={expression} position={[0, 0, 0]} />
